...
@section('main-content')
    <div style="margin: auto; margin-bottom: 10px; width: 80%;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('ad-min.priorities.create') }}">
                    ...
                    @foreach($priorities as $key => $priority)
                        <tr data-entry-id="{{ $priority->id }}">
                            ...
                            <td>
                                    ...
                                    <a class="btn btn-xs btn-primary" href="{{ route('ad-min.priorities.show', $priority->id) }}">
                                        {{ trans('global.view') }}
                                    </a>

                                    <a class="btn btn-xs btn-info" href="{{ route('ad-min.priorities.edit', $priority->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>

                                    <form action="{{ route('ad-min.priorities.destroy', $priority->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('priority_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('ad-min.priorities.massDestroy') }}",
